...
enum S_States{S_start, on2, off2, wait}S_state;
enum Button_States{Button_start, wait2, up, down}Button_state;
...
void TickSL(){
	switch(S_state){//transitons
		...
		case on:
			if((count3 < button) && ((~PINA & 0x04) == 0x04)){
				S_state = on;
				count3++;
			}
